...
describe('FilterSort', () => {
    ...
    it('should show a show more button with a number if there are hidden tags', () => {
        topics = Array.from({ length: 20 }, (v, k) => k + 1).map((i) => ({ name: 'topic' + i }));
        const { getAllByTestId } = render(<FilterSort topics={topics} max={7} onApply={mocks.onApply} />);
    ...
    it('should not display the show more button if under max', () => {
        const { queryByTestId } = render(<FilterSort topics={topics} max={3} onApply={mocks.onApply} />);
    ...
    it('should show all filters when the show more button is clicked', () => {
        ...
        const { getAllByTestId } = render(<FilterSort topics={topics} max={3} onApply={mocks.onApply} />);
